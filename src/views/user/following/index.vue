<template>
  <div class="header">
    <div class="header_text">@{{ username }} 关注中</div>
  </div>
  ...
  <div v-if="!data.length&&!isError" style="text-align: center;color: #dee7e4">此用户未关注任何人~</div>
...
const getFollowingList = async () => {
  ...
  const res=await eventStore.following_list({
    name:username.value
  })
  isLoading.value=false
  if (res){
    data.push(...res.following_list)
    return
  }
  isError.value=true
}
